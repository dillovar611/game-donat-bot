...
Раванд:
  ...
  2. Мизоҷ маблағро мефиристад ва РАСМИ ЧЕКРО ба бот мефиристад (ҳатмӣ —
    ...
    if order_ref:
        ...
        if order and order.get("payment_method") in ("dushanbe_city", "alif", "eskhata"):
            if comment_reused:
                ...
                # маблағ дар банк тасдиқ шуд — худаш «Тасдиқ»-ро занад.
                if abs(float(order["price"]) - summa) > 0.011:
                    await _notify_admins_wrong_amount(message.bot, order, summa, kod)
                    return
    ...
    for m in _SCAN_ENTRY_RE.finditer(text):
        ...
        if emoji == "✅" and order_ref:
            ...
            if abs(float(order["price"]) - summa) > 0.011:
                ...
                continue
            if status == "paid":
                # Дар навбати админ аст — АВТОМАТ намекунем (шояд дастӣ
...
async def _notify_admin_stale_comment(bot: Bot, done_order: dict, summa: float, kod: str):
    ...
    if candidates:
        ...
        lines += (
            f"⚠️ Ин маблағ ба ягон фармоиши нодир худкор мувофиқ НАШУД, аммо ҳамин "
...
async def _build_unfound_context(order: dict, user: dict) -> str:
    ...
    # (1) Ҷустуҷӯи ХУДКОРИ банк — оё маблағи наздик омад?
    bank_hit = None
    try:
    ...
    # (7) Саломатии нотифайери DC (танҳо агар маблағи банк ёфт нашуд — сабабро фаҳмонад)
    if not bank_hit:
        try:
    ...
    if risk == "fraud":
        ...
        head = "🟢 <b>Эҳтимол ДУРУСТ (маблағ дар банк ҳаст)</b>\n"
    elif risk in ("new", "risky"):
        head = "🟠 <b>Диққат — мизоҷи хавфнок ва маблағ дар банк нест</b>\n"
    else:
        head = ""
...
async def _notify_admin_cart_paid(bot: Bot, order: dict):
    ...
    маблағ гум нашавад (вагарна фармоиши 'pending' ноаён мемонд)."""
    ...
    caption = (
        ...
        f"Мизоҷ маблағ дод, вале расми чек нафиристод. Дастӣ тасдиқ кунед "
        f"(то маблағ гум нашавад):\n\n"
        f"🆔 Фармоишҳо: <b>{ids}</b>\n"
...
async def _notify_admin_payment_confirmed(bot: Bot, order: dict, summa: float, kod: str):
    ...
    бошад) — танҳо ба админ тугмаи «Тасдиқ» бо хабари «маблағ омад» медиҳем."""
    order_id = order["id"]
    user = await db.get_user(order["user_id"])
    username = f"@{user['username']}" if user and user.get("username") else "—"
    caption = (
        f"💰 <b>Пардохт дар банк ТАСДИҚ шуд — фармоиши #{order_id}</b>\n\n"
        f"Ин фармоиш дар навбати шумост ва акнун маблағаш дар банк дида шуд "
        f"(<b>{summa:.2f} сом</b>).\n\n"
        f"👤 Харидор: {esc(user.get('full_name') if user else '—')} ({esc(username)})\n"
        f"🆔 ID: <code>{order['user_id']}</code>\n"
        f"🎁 {order['label']} → <code>{order['game_id']}</code>\n\n"
        f"⚠️ Агар шумо инро аллакай ДАСТӢ иҷро карда бошед — «Рад» кунед "
        f"(то дучанд нашавад). Вагарна «Тасдиқ»."
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Тасдиқ — донат кун", callback_data=_confirm_cb(order))],
        [InlineKeyboardButton(text="❌ Рад — дастӣ иҷро шуд", callback_data=f"no_{order_id}")],
    ])
    for admin_id in config.ADMIN_IDS:
        try:
            ...
            logger.error(f"Огоҳии пардохти тасдиқшуда ба {admin_id} нарасид: {e}")
    ...
    try:
        if api_order_id:
            customer_text = (
                ...
                f"ба шумо хабар медиҳем. Маблағатон бехатар аст. 🙏"
            )
        else:
            customer_text = (
                ...
                f"дастӣ ҳал мекунад. Маблағатон бехатар аст. 🙏"
            )
...
async def _admin_report_failure(bot: Bot, order: dict, kod: str, api_order_id: str, uncertain: bool = False):
    ...
    text = (
        ...
        f"{id_check_line}"
        f"{warning_line}\n"
        f"Маблағи мизоҷ ҚАБУЛ шудааст — ҳатман ҳал кунед!"
    )
    for admin_id in config.ADMIN_IDS:
        try:
...
async def run_donate_inner(bot: Bot, order: dict, kod: str):
    ...
    if success:
        ...
        try:
            await bot.send_message(
                user_id,
                f"{db.anim('success', pemoji.pe(pemoji.PARTY, '🎉') + ' <b>Донат анҷом ёфт!</b>')} <b>Алмазҳо фиристода шуданд!</b>\n\n"
                f"🆔 Фармоиш: #{order_id}\n"
                f"🎁 {order['label']} → <code>{order['game_id']}</code>\n\n"
...
async def run_donate_from_balance(bot: Bot, order: dict):
    ...
    if success:
        ...
        try:
            await bot.send_message(
                user_id,
                f"{db.anim('success', pemoji.pe(pemoji.PARTY, '🎉') + ' <b>Донат анҷом ёфт!</b>')} <b>{esc(order['label'])} фиристода шуд!</b>\n\n"
                f"🆔 Фармоиш: #{order_id}\n\n"
                f"🙏 Ташаккур барои харид!\n\n"
                f"{db.anim('review', pemoji.pe(pemoji.STAR, '⭐') + ' Лутфан отзив гузоред')}:",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def run_donate_group_from_balance(bot: Bot, orders: list):
    ...
    if ok_items:
        ...
        try:
            await bot.send_message(
                user_id,
                f"{db.anim('success', pemoji.pe(pemoji.PARTY, '🎉') + ' <b>Донат анҷом ёфт!</b>')} <b>Маҳсулотҳо фиристода шуданд!</b>\n\n"
                f"{lines}\n\n🙏 Ташаккур барои харид!\n\n{db.anim('review', pemoji.pe(pemoji.STAR, '⭐') + ' Лутфан отзив гузоред')}:",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="⭐ Отзив гузоштан",
                                          callback_data=f"review_{ok_items[0]['id']}")]
                ]), parse_mode="HTML")
                ...
                try:
                    await bot.send_message(
                        ...
                        pemoji.premiumize(
                            ...
                            f"Ҳељ маблағе кам нашуд — хавотир нашав.\n\n"
            ...
            for order in stale:
                try:
                    await bot.send_message(
                        ...
                        f"🛒 Агар пардохт накарда бошед — хавотир нашавед, ҳељ маблағе кам "
        ...
        try:
            ...
            for order in unfound:
                ...
                caption = (
                    ...
                    f"🎁 {order['label']} → <code>{order['game_id']}</code>\n\n"
                    f"{extra}\n\n"
                    f"Чекро тафтиш кунед: агар маблағ воқеан омада бошад — «Тасдиқ»."
                )
...
async def _complete_pending_purchase(bot: Bot, user_id: int, pending: dict):
    ...
    # маблағ ба баланс ХУДКОР БАРНАМЕГАРДАД (қоидаи соҳиб). Ба ҷои он админ
    ...
    if success:
        ...
        try:
            await bot.send_message(
                ...
                f"интихоб кард ва тӯҳфаро БЕМАБЛАҒ фиристод! 🎊\n\n"
                f"🎁 Тӯҳфа: <b>{label}</b>\n"
        ...
        try:
            ...
            caption = (
                ...
                f"4️⃣ Ӯ тӯҳфаро <b>БЕМАБЛАҒ</b> мегирад\n\n"
                f"❗️ <b>Дархост кардан лозим НЕСТ</b>\n"
                f"❗️ <b>Маблағи иловагӣ додан лозим НЕСТ</b>\n"
    ...
    while True:
        try:
            ...
            for order in orders:
                ...
                # ин маблағ дар «сӯрохи» афтодааст. Дар ин ҳолат ЯК бор ба админ
                # хабар медиҳем (claim_admin_alert такрорро мебандад), то маблағи
                # мизоҷ гум нашавад.
                created_at = order.get("created_at")
...
async def auto_archive_loop(bot: Bot, interval_seconds: int = 300):
    ...
    ҳељ як ҳисобот ва ҳисоби фоида онро намегирад, маблағ гум намешавад.
    """
                        ...
                        for admin_id in config.ADMIN_IDS:
                            try:
                                await bot.send_message(
                                    ...
                                    f"🔒 Ҳељ чиз нест нашуд — маблағ, чек ва "
...
async def giveaway_loop(bot: Bot, interval_seconds: int = 60):
    ...
    як барандаи тасодуфиро интихоб карда, маҳсулоти танзимшударо ба ӯ БЕМАБЛАҒ
