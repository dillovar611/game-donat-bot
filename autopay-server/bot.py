...
async def giveaway_info(call: CallbackQuery):
    ...
    await _safe_edit(
        ...
        "интихоб мекунад\n"
        "4️⃣ Ӯ маҳсулотро <b>БЕМАБЛАҒ</b> мегирад 🍀\n\n"
        "❗️ Дархост кардан лозим НЕСТ\n"
        "❗️ Маблағи иловагӣ додан лозим НЕСТ\n"
...
_FAQ_ITEMS = [
    ...
    ("💰 Маблағ баргардонида мешавад?",
     "Не, баъд аз донати муваффақ маблағ бозгардонида намешавад (ба ғайр аз "
    ...
    ("🎁 Тӯҳфаи ройгон чист?",
     ...
     "як нафар тасодуфан интихоб мешавад ва тӯҳфаро БЕМАБЛАҒ мегирад.\n\n"
     "Дархост кардан, маблағи иловагӣ додан ё VIP шудан лозим НЕСТ — "
     "фақат харид кунед."),
]
...
TERMS_TEXT = (
    ...
    "3️⃣ <b>Бозгашти маблағ:</b> Пас аз донат маблағ бозгардонида <b>намешавад</b>.\n\n"
...
def _welcome_text(user, greeted: bool = True) -> str:
    ...
    # Сарлавҳаи аниматсионӣ аз кэши муштараки database (як нусха барои ҳама).
    _title_val = getattr(db, "welcome_title_cache", "") or ""
    title = f"{_title_val}\n\n" if _title_val else ""
    hello = (
        ...
        try:
            ...
            for order in await db.get_stale_paid_orders(
                ...
                try:
                    ...
                    # ЭЗОҲ: барои фармоиши ОДДӢ паёми дуюм ба админ НЕСТ (вагарна
                    # ду бор меомад — _watchdog_loop ҷамъбастӣ хабар медиҳад).
                    # ВАЛЕ САБАД (гурӯҳ) ХУДКОР донат намешавад — ҲАТМАН тасдиқи
                    # админро мехоҳад. Агар огоҳии аввал гум шуда бошад, сабад
                    # ятим мемонад. Пас барои САБАД БОЗ ба админ бо тугмаи
                    # «Тасдиқи гурӯҳ» мефиристем (як бор — деддуп бо _seen_groups).
                    if _gid:
                        try:
                            await autopay._notify_admin_cart_awaiting(bot, order)
                        except Exception as e:
                            logger.error(f"Огоҳии тасдиқи сабад #{order_id} нашуд: {e}")
                except Exception as e:
                    logger.error(f"Коркарди ёдоварии фармоиши #{order_id} нашуд: {e}")

            # ---- Огоҳии ТАЪХИРИИ «пардохти сабад омад — чек нарасид» ----
            # Маблағ омад (kod резерв) вале мизоҷ то ~5 дақ чек нафиристод —
...
async def _money_safety_loop(bot: Bot):
    """ТӮРИ БЕХАТАРИИ УМУМӢ ЗИДДИ ГУМ ШУДАНИ МАБЛАҒ (ҳар 10 дақиқа).
    ...
    афтоданд (маблағ омад, вале донат/тасдиқ нашуд ва админ бехабар монд).
    ...
    while True:
        ...
        try:
            ...
            text = (
                f"🛟 <b>ТӮРИ БЕХАТАРӢ: маблағи ноанҷом!</b>\n\n"
                f"<b>{len(claimed)}</b> фармоиш маблағ гирифтаанд, вале то ҳол ба "
                f"ниҳоят нарасидаанд (шояд пеш аз restart дар сӯрохи афтода "
                f"буданд):\n\n" + "\n".join(lines) + more + "\n\n"
                f"Лутфан онҳоро дастӣ санҷед — то ягон маблағ гум нашавад."
            )
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📋 Фармоишҳои интизорӣ", callback_data="a_pending_orders")],
            ])
...
async def _post_deploy_report(bot: Bot):
    ...
    Баъди ҳар restart ба админ мегӯяд: чанд фармоиши МАБЛАҒ-гирифтаи ноанҷом
    ...
    try:
        ...
        text = (
            ...
            f"Ҳозир <b>{n}</b> фармоиши маблағ-гирифта то ҳол ноанҷом аст "
...
# Вақте мизоҷ дар вақти интизорӣ чизе менависад ("маблағам чӣ шуд?", "алмос
...
_ORDER_STATUS_REPLY = {
    "awaiting_autopay": (
        ...
        "Маблағро фиристодед? Пас расми чекро ба ҳамин ҷо фиристед — "
    ...
    "failed": (
        ...
        "шавад, ба шумо хабар медиҳад. <b>Маблағатон бехатар аст</b> — "
...
_TOPUP_STATUS_REPLY = {
    ...
    "failed": (
        ...
        "<b>Маблағатон бехатар аст</b> — ҳељ ҷо гум намешавад. 🙏"
    ),
}
...
async def main():
    ...
    # Тӯри бехатарии УМУМӢ зидди гум шудани маблағ (зидди сӯрохи restart) — ҳар 10 дақ
