...
# Лаҳзаи оғози бот. Фармоишҳое, ки ПЕШ аз ин сохта шудаанд (яъне ҳангоми
# кори версияи кӯҳна ноком монда буданд), метавонанд аллакай ДАСТӢ ҳал
# шуда бошанд — вале дар база ҳанӯз 'failed' истода бошанд. Барои онҳо
# такрори ХУДКОР сохта НАМЕШАВАД (вагарна пул беҳуда харҷ мешавад ва
# мизоҷ дубора алмос мегирад). Танҳо санҷиши бехатари "оё иҷро шудааст?"
# барояшон кор мекунад.
...
async def recheck_loop(bot: Bot, interval_seconds: int = 180):
    ...
    мешавад: агар FazerCards ХУДАШ "рад шуд" гуфта бошад (ҳолати ниҳоии
    маълум) ВА фармоиш баъд аз оғози ин версияи бот сохта шуда бошад.

    Фармоишҳои КӮҲНА (пеш аз рестарт) ҳељ гоҳ такрори худкор намегиранд —
    шояд соҳиб онҳоро аллакай дастӣ ҳал карда бошад. Барояшон танҳо
    санҷиши бехатари "оё дар асл иҷро шудааст?" кор мекунад.
    """
    ...
    while True:
        try:
            ...
            for order in orders:
                ...
                # Барояш танҳо санҷиши бехатар кор мекунад — ниг. _BOT_START_TS
                created_at = order.get("created_at")
                is_old = bool(created_at and created_at < _BOT_START_TS)
                ...
                if state == "completed":
                    ...
                    _recheck_settled.add(order_id)
                    if is_old:
                        # Фармоиши КӮҲНА (пеш аз оғози ин версия) — шояд
                        # соҳиб онро аллакай дастӣ ҳал карда бошад.
                        # Такрори худкор НАМЕКУНЕМ ва паём ҳам намефиристем —
                        # он дар «🛠 Кор барои ман» аллакай дида мешавад.
                        logger.info(
                            f"recheck_loop: #{order_id} кӯҳна аст — такрори "
                            f"худкор НАШУД (эҳтимол дастӣ ҳал шудааст)"
                        )
                    elif await db.claim_failed_order_for_autoretry(order_id):
                        logger.info(
                    ...
                    if is_old:
                        # Фармоиши кӯҳна — паём намефиристем, вагарна ҳангоми
                        # рестарт як даста огоҳии кӯҳна якбора меборад
                        continue
