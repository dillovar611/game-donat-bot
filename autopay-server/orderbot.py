...
def _is_last_order_query(text: str) -> bool:
    ...
    subjects = ("фармоиш", "заказ", "алмос", "almos", "diamond", "олмос",
                "uc", "юси", "харид", "покупк", "донат")
    if not any(s in lower for s in subjects):
        return False
    triggers = ("охирин", "чи шуд", "чӣ шуд", "кай", "куҷо", "куҷост",
                "ҳолат", "холат", "хабар", "наомад", "нарасид", "намеояд",
                "нест шуд", "чаро", "где", "не пришл", "не дошл", "не получ")
