...
async def _balance_pay_cart(call: CallbackQuery, uid: int, data: dict,
    ...
    маблағ ХУДКОР барнамегардад (қоидаи соҳиб) — ба ҷои он админ огоҳ
        ...
        for admin_id in config.ADMIN_IDS:
            try:
                await call.bot.send_message(
                    admin_id,
                    f"⚠️ <b>Маблағ аз баланс кам шуд, вале фармоиш сохта НАШУД!</b>\n\n"
...
async def buy_enter_id(message: Message, state: FSMContext):
    ...
    if nickname:
        ...
        # API номро наёфт — огоҳии ТАРСНОК (то мизоҷ ба ID-и хато маблағ надиҳад)
        text = (
            f"🔥 <b>Free Fire</b>\n\n"
            f"🆔 ID: <code>{player_id}</code>\n\n"
            f"⛔️ <b>ДИҚҚАТ! Номи ин аккаунт ЁФТ НАШУД!</b>\n\n"
            f"Ин метавонад маънои онро дошта бошад, ки ID <b>ХАТО</b> аст.\n"
            f"🔴 Агар ID нодуруст бошаду шумо маблағ диҳед — <b>МАБЛАҒАТОН МЕСӮЗАД</b> "
...
async def _combo_levelup_warning(combo_id: int | None) -> str:
    ...
    зада мешавад ва агар такрор харанд, маблағашон месӯзад."""
    if not combo_id:
        return ""
    try:
    ...
    return (
        ...
        "<b>МАБЛАҒАТОН МЕСӮЗАД</b> ва баргардонида НАМЕШАВАД!\n\n"
...
# рақами ТЕЛЕФОН мебурд (account=929998174) — мизоҷон иштибоҳ карда маблағро
...
async def _autopay_requisites(call: CallbackQuery, state: FSMContext, data: dict,
    ...
    # Оффери БАРҚӢ — агар маҳсули ҷорӣ ҳамон маҳсули оффер бошад ва мӯҳлат
    # нагузашта (танҳо агар тахфифи №1 татбиқ нашуда бошад — то дучанд нашавад)
    if not reoffer_applied:
        try:
            ...
            logger.error(f"flash discount хато: {e}")
...
async def _autopay_receive_check(message: Message, data: dict) -> bool:
    ...
    # Агар худи фармоиши FSM ҳанӯз маблағ надошта бошад, вале мизоҷ фармоиши
    # ДИГАРЕ дошта бошад, ки маблағаш дар банк ёфт шуда (kod резерв) — чекро ба
    # ҲАМОН мебандем. Ин ҳолати «мизоҷ ба #1111 маблағ дод, вале #1112 сохта,
    ...
    # фиристад, маблағ нафиристад ва баъди эскалатсия донати ройгон гирад.
...
async def receive_check(message: Message, state: FSMContext):
    ...
    # РОУТИНГИ ДУРУСТИ ЧЕК: агар фармоиши FSM маблағ надошта бошад, вале мизоҷ
    # фармоиши дигаре дошта бошад, ки маблағаш ёфт шуда (kod резерв) — чекро ба
    # ҲАМОН мебандем (мизоҷ ба #1111 маблағ дод, #1112 сохта чекро ба #1112 дод).
    import autopay
...
async def pay_with_balance(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # дидашуда, ки дар state мондааст. Яъне мизоҷ барои 10 баста маблағ
...
async def standoff_pay_balance(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # сохтани фармоиш хато диҳад, маблағ БЕСАДО гум нашавад, балки админ
...
# ════════════════════════════════════════════════════════
# Мизоҷ платформаро интихоб мекунад → видеои намоишӣ мебинад → маблағ медиҳад
