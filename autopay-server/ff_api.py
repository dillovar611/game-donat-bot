...
# Возврат ҳолати НИҲОӢ аст: маблағ баргашт, донат нашуд — интизорӣ бефоида.
...
async def peek_order_status(api_order_id: str):
    ...
    намесозад ва ҳеҷ маблағ харҷ намекунад. Барои тафтишгари худкори
...
async def get_nickname_ffid(player_id: str) -> str:
    ...
    Санҷиш танҳо ХОНДАН аст — на маблағ мехӯрад, на чизе месозад. Агар
...
async def find_ml_fields(player_id: str, server_id: str,
    ...
    Ин санҷиш ТАНҲО validate-id-ро истифода мебарад — яъне ҳељ маблағ сарф
    намешавад ва ҳељ донат намешавад. Лозим аст, чунки бо номи НОДУРУСТИ
    майдон донат ноком мешавад, вале маблағи мизоҷ аллакай гирифта шудааст.
